    ...
    AccessControlUpgradeable
{
    ...
    bytes32 public constant EXCHANGE_UPDATE_ROLE =
        keccak256("EXCHANGE_UPDATE_ROLE");
    bytes32 public constant MANAGE_STAKING_CONFIG_ROLE =
        keccak256("MANAGE_STAKING_CONFIG_ROLE");
    bytes32 public constant TOKEN_SAFE_PULL_ROLE =
        keccak256("TOKEN_SAFE_PULL_ROLE");
    bytes32 public constant APPROVE_WITHDRAWAL_ROLE =
        keccak256("APPROVE_WITHDRAWAL_ROLE");

    modifier canPauseRole() {
        require(
            hasRole(PAUSE_ROLE, msg.sender),
            "Caller does not have PAUSE_ROLE"
        );
        _;
    }
    ...
    function setServiceFee(
        uint256 _serviceFee
    ) public hasManageStakingConfigRole {
        require(
            _serviceFee <= 0.01 ether,
            "Service fee cannot be more than 0.01 ETH"
        );
    ...
    function setWithdrawalManager(
        ...
        require(
            ...
            "Withdrawal manager cannot be null"
        );
    ...
    function setMaxSupply(
        ...
        require(
            ...
            "Max supply must be greater than the current total supply"
        );
     ...
     *
     */
    function setMinStakingAmt(
        uint256 _minStakingAmt
    ) public hasManageStakingConfigRole {
        require(
            _minStakingAmt > bridgingFee,
            "Min staking amount must be more than bridging fee"
        );
    ...
    function setStakingFee(
        ...
        require(
            _stakingFee < 20,
            "Staking fee cannot be more than equal to 20"
        );
    ...
    function setBridgingFee(
        ...
        require(
            _bridgingFee <= 0.2 gwei,
            "Bridging fee cannot be more than 0.2 TAO"
        );
     ...
     *
     */
    function setMaxDepositPerRequest(
        uint256 _maxDepositPerRequest
    ) public hasManageStakingConfigRole {
        require(
            _maxDepositPerRequest > 0,
            "Max deposit per request must be more than 0"
        );
     ...
     *
     */
    function setMaxUnstakeRequest(
        uint256 _maxUnstakeRequests
    ) public hasManageStakingConfigRole {
        require(
            _maxUnstakeRequests > 0,
            "Max unstake requests must be more than 0"
        );
    ...
    function setLowerExchangeRateBound(
        uint256 _newLowerBound
    ) public hasExchangeUpdateRole {
        require(_newLowerBound > 0, "New lower bound must be more than 0");
        require(
            _newLowerBound < upperExchangeRateBound ||
                upperExchangeRateBound == 0,
    ...
    function setUpperExchangeRateBound(
        uint256 _newUpperBound
    ) public hasExchangeUpdateRole {
    ...
    function setUnstakingFee(
        ...
        require(
            _unstakingFee <= 0.2 gwei,
            "Unstaking fee cannot be more than 0.2 TAO"
        );
    ...
    function setWTAO(
        address _wTAO
    ) public hasManageStakingConfigRole nonReentrant {
     ...
     */
    function setNativeTokenReceiver(
        string memory _nativeWalletReceiver
    ) public hasManageStakingConfigRole {
     ...
     *
     */
    function getWntdTAObyWTAO(
        uint256 wtaoAmount
    ) public view returns (uint256) {
        return (wtaoAmount * exchangeRate) / 1 ether;
    }

    function getWTAOByWntdTAO(
        uint256 wntdTaoAmount
    ) public view returns (uint256) {
        return (wntdTaoAmount * 1 ether) / exchangeRate;
    }

    function getWTAOByWntdTAOAfterFee(
        uint256 wntdTaoAmount
    ) public view returns (uint256) {
     ...
     *
     */
    function _checkValidFinneyWallet(
        string memory _nativeWalletReceiver
    ) internal pure {
        require(
    ...
    function requestUnstake(
        uint256 wntdTAOAmt
    ) public payable nonReentrant checkPaused {
        ...
        require(
            balanceOf(msg.sender) >= wntdTAOAmt,
            "Insufficient wntdTAO balance"
        );
    ...
    function getUnstakeRequestByUser(
        address user
     ...
     *
     */
    function approveMultipleUnstakes(
        UserRequest[] calldata requests
    ) public hasApproveWithdrawalRole nonReentrant checkPaused {
        ...
        address commonWrappedToken = unstakeRequests[requests[0].user][
            requests[0].requestIndex
        ].wrappedToken;
        ...
        for (uint256 i = 0; i < requests.length; i++) {
            ...
            require(
                unstakeRequests[request.user][request.requestIndex]
                    .wrappedToken == commonWrappedToken,
                "Wrapped token is not the same across all unstake requests"
            );

            totalRequiredTaoAmt += unstakeRequests[request.user][
    ...
    function unstake(uint256 requestIndex) public nonReentrant checkPaused {
        ...
        IERC20(request.wrappedToken).safeTransfer(msg.sender, amountToTransfer);
    ...
    function updateExchangeRate(uint256 newRate) public hasExchangeUpdateRole {
        require(
            newRate >= lowerExchangeRateBound &&
                newRate <= upperExchangeRateBound,
    ...
    function calculateAmtAfterFee(
        ...
        require(
            amountAfterTotalFees > 0,
            "Wrap amount after fee must be more than 0"
        );
    ...
    function maxTaoForWrap() public view returns (uint256) {
        ...
        uint256 equivalentTaoAmount = (availableNtdTAOSupply * 1 ether) /
            exchangeRate;
    ...
    function wrap(uint256 wtaoAmount) public checkPaused {
        ...
        require(
            wtaoAmount > minStakingAmt,
            "Does not meet minimum staking amount"
        );
